...
def history_window_for_t0(
    xyz: np.ndarray,
    quat_xyzw: np.ndarray,
    t0_idx: int,
    history_steps: int,
    previous_xyz: np.ndarray | None = None,
    previous_quat_xyzw: np.ndarray | None = None,
) -> tuple[np.ndarray, np.ndarray]:
    """Return up to history_steps ending at t0, borrowing from the previous clip."""
    current_start = max(0, int(t0_idx) - int(history_steps) + 1)
    history_xyz = xyz[current_start : int(t0_idx) + 1]
    history_quat = quat_xyzw[current_start : int(t0_idx) + 1]
    missing = int(history_steps) - len(history_xyz)
    if missing > 0 and previous_xyz is not None and previous_quat_xyzw is not None:
        history_xyz = np.vstack([previous_xyz[-missing:], history_xyz])
        history_quat = np.vstack([previous_quat_xyzw[-missing:], history_quat])
    return history_xyz, history_quat
...
def export_future_xy_txt(
    ...
    stats = ExportStats()
    traj_id = 0
    previous_context_by_dataset: dict[str, tuple[np.ndarray, np.ndarray]] = {}
    ...
    with data_txt.open("w", encoding="utf-8") as f:
        ...
        for parquet_file in iter_egomotion_files(train_data_root):
            ...
            if not required.issubset(df.columns) or len(df) < steps + 1:
            ...
            previous_xyz, previous_quat = previous_context_by_dataset.get(
                dataset,
                (None, None),
            )

            first_t0 = 0 if previous_xyz is not None else int(history_steps) - 1
            ...
            for t0_idx in range(first_t0, last_t0 + 1, max(1, t0_stride)):
                ...
                history_xyz, history_quat = history_window_for_t0(
                    xyz,
                    quat_xyzw,
                    t0_idx,
                    history_steps,
                ...
                if len(history_xyz) == 0 or len(future_xyz) != steps:
                    stats.skipped_short_or_bad += 1
                    continue

                t0_rot_inv = estimate_t0_rotation(history_xyz, history_quat).inv()
            ...
            previous_context_by_dataset[dataset] = (xyz, quat_xyzw)
