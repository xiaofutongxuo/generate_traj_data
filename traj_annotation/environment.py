"""Runtime environment setup for the trajectory annotator GUI."""
...
def setup_environment(platform_name: str | None = None) -> None:
    """Apply GUI environment defaults without loading inference dependencies."""
    is_windows = _is_windows_platform(platform_name)
    os.environ.setdefault("GENERATE_TRAJ_USE_TORCH", "0")
    if not is_windows and not os.environ.get("DISPLAY") and Path("/tmp/.X11-unix/X1").exists():
